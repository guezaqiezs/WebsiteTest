...
/*	Property: validateField
	...
	*/
	validateField: function(field, force){
		...
		var warned = ''; // var warned = this.form.getElement('.warning');
		...
		if(field && (!failed || force || field == failed || (failed && !this.options.serial))){
			...
			if (validators && !field.hasClass('warnOnly')){
				if (field.hasClass('required-enabled')) { field.removeClass('required-enabled'); }
				if (field.hasClass('required-disabled')) { field.removeClass('required-disabled'); } 
...
/*	Property: required
	...
	*/
	['required', {
		errorMsg: function(element){return 'Ce champ est obligatoire.' }, 
		test: function(element) { 
...
/*	Property: minLength
	...
	*/
	['minLength', {
		errorMsg: function(element, props){
			if($type(props.minLength))
				return 'Merci de saisir au moins ' + props.minLength + ' caractères (vous avez saisi ' + element.getValue().length + ' caractères).';
			else return '';
		}, 
		test: function(element, props) {
	...
	*/
	['maxLength', {
		errorMsg: function(element, props){
			...
			if($type(props.maxLength))
				return 'Merci de ne pas saisir plus de ' + props.maxLength + ' caractères (vous avez saisi ' + element.getValue().length + ' caractères).';
			else return '';
		}, 
		test: function(element, props) {
			//if the value is <= than the maxLength value, element passes test
...
/*	Property: validate-number
	...
	*/	
	['validate-number', {
		errorMsg: 'Merci de saisir un nombre valide.',
		test: function(element) {
...
/*	Property: validate-digits
	...
	*/
	['validate-digits', {
		errorMsg: 'Merci de n\'utiliser que des chiffres. Eviter les espaces ou autres caractères spéciaux comme la virgule, le tiret...', 
		test: function(element) {
			return FormValidator.getValidator('IsEmpty').test(element) || 
...
/*	Property: validate-alpha
		...
		Error Msg - 'Please use letters only (a-zA-Z) in this field.'
	*/
	['validate-alpha', {
		errorMsg: 'Merci de n\'utiliser que des lettres (a-zA-Z).', 
		test: function (element) {
			return FormValidator.getValidator('IsEmpty').test(element) ||  /^[a-zA-Z]+$/.test(element.getValue())
		}
	}],
	/*	Property: validate-alpha-lower
		Validates that the entry contains only letters 

		Error Msg - 'Please use letters only (a-z) in this field.'
	*/
	['validate-alpha-lower', {
		errorMsg: 'Merci de n\'utiliser que des lettres en minuscule (a-z).', 
		test: function (element) {
			return FormValidator.getValidator('IsEmpty').test(element) ||  /^[a-z]+$/.test(element.getValue())
		}
	}],
/*	Property: validate-alphanum
	...
	*/
	['validate-alphanum', {
		errorMsg: 'Merci de n\'utiliser  que des lettres (a-zA-Z) ou des chiffres (0-9). Pas d\'espace, ni de caractères spéciaux comme la virgule', 
		test: function(element) {
			return FormValidator.getValidator('IsEmpty').test(element) || !/\W/.test(element.getValue())
		}
	}],
	/*	Property: validate-alphanum-lower-under
		Validates that the entry contains only letters 

		Error Msg - 'Please use letters only (a-z) in this field.'
	*/
	['validate-alphanum-lower-under', {
		errorMsg: 'Merci de n\'utiliser que des lettres en minuscule et des chiffres (a-z0-9_).', 
		test: function (element) {
			return FormValidator.getValidator('IsEmpty').test(element) ||  /^[a-z0-9_]+$/.test(element.getValue())
		}
	}],
	...
	['validate-date', {
		...
		errorMsg: function(element, props) {
			if (Date.parse) {
				...
				return 'Merci de saisir une date valide comme ' + new Date().format(format);
			} else {
				return 'Merci de saisir une date valide comme MM/DD/YYYY (i.e. "31/12/1999")';
			}
		},
		test: function(element, props) {
			...
			if (Date.parse) {
				var format = props.dateFormat || "%x";
				var d = Date.parse(element.getValue());
				var formatted = d.format(format);
				if (formatted != "invalid date") element.setProperty('value', formatted);
				return !isNaN(d);
			} else {
...
/*	Property: validate-email
		...
		Error Msg - 'Please enter a valid email address. For example name@domain.com .'
	*/
	['validate-email', {
		errorMsg: 'Merci de saisir une adresse email valide. Par exemple, name@domain.com .', 
		test: function (element) {
...
/*	Property: validate-url
	...
	*/
	['validate-url', {
		errorMsg: 'Merci de saisir une URL correcte.', 
		test: function (element) {
...
/*	Property: validate-date-au
	...
	*/
	...
	['validate-date-au', {
		errorMsg: 'Merci d\'utiliser ce format de date : dd/mm/yyyy. Par exemple 17/03/2006 pour le 17 Mars 2006.',
		test: function(element) {
			if(FormValidator.getValidator('IsEmpty').test(element)) return true;
	    var regex = /^(\d{2})\/(\d{2})\/(\d{4})$/;
	    if(!regex.test(element.getValue())) return false;
...
/*	Property: validate-currency-dollar
	...
	*/
	['validate-currency-dollar', {
		errorMsg: 'Merci de saisir un montant correct. Par exemple $100.00 .', 
		test: function(element) {
			// [$]1[##][,###]+[.##]
			// [$]1###+[.##]
			// [$]0.##
...
Revision 1.26  2008/01/07 20:45:16  newtona
date picker and form validator now play together a bit more nicely; they share output formats so if you set one in the validatorProps, it'll overwrite the option for the date picker, but if you don't set one for validatorProps, the date picker will write the validatorProps for you.

Revision 1.25  2008/01/07 19:47:32  newtona
