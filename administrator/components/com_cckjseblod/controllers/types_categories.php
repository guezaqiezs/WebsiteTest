...
/**
 * Types_Categories		Controller Class
 **/
class CCKjSeblodControllerTypes_Categories extends CCKjSeblodController
{
	/**
	 * Vars
	 **/
	...
	function display()
	{
		...
		if ( empty( $view ) ) {
			JRequest::setVar( 'view', 'types_categories' );
	...
	{	
		...
		switch( $this->getTask() ) {
			case 'add':
			case 'edit':
				JRequest::setVar( 'view', 'types_category' );
	...
	function remove()
	{
		...
		$deleteMode	=	1;
		$model		=	$this->getModel( 'types_category' );
	...
	function save()
	{
		...
		switch( $this->getTask() ) {
			case 'apply':
				$link	=	( $rowId ) ? _LINK_CCKJSEBLOD_TYPES_CATEGORIES.'&task=edit&cid[]='.$rowId : _LINK_CCKJSEBLOD_TYPES;
				break;
			case 'save':
				$link	=	_LINK_CCKJSEBLOD_TYPES_CATEGORIES;
				break;
