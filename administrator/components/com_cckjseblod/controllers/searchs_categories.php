...
/**
 * Searchs_Categories		Controller Class
 **/
class CCKjSeblodControllerSearchs_Categories extends CCKjSeblodController
{
	/**
	 * Vars
	 **/
	...
	function display()
	{
		...
		if ( empty( $view ) ) {
			JRequest::setVar( 'view', 'searchs_categories' );
	...
	{	
		...
		switch( $this->getTask() ) {
			case 'add':
			case 'edit':
				JRequest::setVar( 'view', 'searchs_category' );
	...
	function remove()
	{
		...
		$deleteMode	=	1;
		$model		=	$this->getModel( 'searchs_category' );
	...
	function save()
	{
		...
		switch( $this->getTask() ) {
			case 'apply':
				$link	=	( $rowId ) ? _LINK_CCKJSEBLOD_SEARCHS_CATEGORIES.'&task=edit&cid[]='.$rowId : _LINK_CCKJSEBLOD_SEARCHS;
				break;
			case 'save':
				$link	=	_LINK_CCKJSEBLOD_SEARCHS_CATEGORIES;
				break;
