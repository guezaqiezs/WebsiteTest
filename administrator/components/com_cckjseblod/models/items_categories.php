...
/**
 * Items_Categories		Model Class
 **/
class CCKjSeblodModelItems_Categories extends JModel
{
	/**
	 * Vars
	 **/
	...
	function getData()
	{
		if ( empty( $this->_data ) )
		{
			...
			$contentItems	=	$this->_db->loadObjectList( 'category' );
			
			if ( sizeof( $this->_data ) ) {
				foreach ( $this->_data as $item ) {
					$item->displayItems	=	@$contentItems[$item->id]->num ? $contentItems[$item->id]->num : 0;
				}	
			}
		}
	...
	function _buildQuery()
	{
		...
		$query	= ' SELECT s.title, s.name, s.color, s.introchar, s.colorchar, ( COUNT( parent.title ) - 1 ) AS depth, s.published, s.lft, s.rgt, s.id, s.display, s.checked_out, s.checked_out_time, s.checked_out AS editor'
