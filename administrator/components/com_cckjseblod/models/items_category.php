...
/**
 * Items_Category	Model Class
 **/
class CCKjSeblodModelItems_Category extends JModel
{
	/**
	 * Vars
	 **/
	...
	{
		if ( empty( $this->_data ) ) {
			$row	=&	$this->getTable( 'items_categories' );
	...
	function store()
	{
		...
		: ( ( ! $data['id'] ) ? HelperjSeblod_Helper::getWysiwygContent( 'description', 'items_categories', $data['cid'][0] ) : HelperjSeblod_Helper::getWysiwygContent( 'description', 'items_categories', $data['id'] ) );
		...
		if ( ( $data['parentid'] != $data['parentdb'] )
			...
			//LOCK TABLE #__jseblod_cck_items_categories WRITE;
			...
			if ( $key == 0 ) {
				$query 	= 'SELECT lft FROM #__jseblod_cck_items_categories'
						. ' WHERE name = "'.$parent.'"'
						;
			} else {
				$bigbrother	=	$brothers[$key - 1];
				$query	= 'SELECT rgt FROM #__jseblod_cck_items_categories'
			...
			$query 	= 'UPDATE #__jseblod_cck_items_categories'
	...
	function delete( $deleteMode )
	{
		...
		if ( $n = count( $cids ) )
		{
			foreach( $cids as $cid ) {
				...
				//$query	= 'SELECT COUNT(s.id) FROM #__jseblod_cck_items AS s'
						//. ' WHERE s.category = '.$cid
						//;
				//$this->_db->setQuery( $query );
				//$gotItems	=	$this->_db->loadResult();
			
				//if ( ! $gotItems ) {
					
					//LOCK TABLE nested_category WRITE;
					
					$query 	= ' SELECT lft, rgt, ( rgt - lft + 1 ) AS width FROM #__jseblod_cck_items_categories'
							. ' WHERE id = '.$cid
							;   
					$this->_db->setQuery( $query );
					...
					if ( $deleteMode == -1 ) {
						if ( ! ( $route[1] - $route[0] == 1 ) ) {				  
							$query 	= 'DELETE FROM #__jseblod_cck_items_categories'
