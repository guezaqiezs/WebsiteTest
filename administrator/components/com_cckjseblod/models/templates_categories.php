...
/**
 * Templates_Categories		Model Class
 **/
class CCKjSeblodModelTemplates_Categories extends JModel
{
	/**
	 * Vars
	 **/
	...
	function getData()
	{
		if ( empty( $this->_data ) )
		{
			...
			$contentTemplates	=	$this->_db->loadObjectList( 'category' );
			if ( sizeof( $this->_data ) ) {
				foreach ( $this->_data as $item ) {
					$item->displayTemplates	=	@$contentTemplates[$item->id]->num ? $contentTemplates[$item->id]->num : 0;
				}
			}
		}
