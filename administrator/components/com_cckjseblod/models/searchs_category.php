...
/**
 * Searchs_Category	Model Class
 **/
class CCKjSeblodModelSearchs_Category extends JModel
{
	/**
	 * Vars
	 **/
	...
	{
		if ( empty( $this->_data ) ) {
			$row	=&	$this->getTable( 'searchs_categories' );
	...
	function _buildQuery( $parentid )
	{
		$query  = 'SELECT s.name, (COUNT(parent.name) - (sub_tree.depth + 1)) AS depth'
				...
				. ' #__jseblod_cck_searchs_categories AS sub_parent,'
				. ' ('
	...
	function store()
	{
		...
		: ( ( ! $data['id'] ) ? HelperjSeblod_Helper::getWysiwygContent( 'description', 'searchs_categories', $data['cid'][0] ) : HelperjSeblod_Helper::getWysiwygContent( 'description', 'searchs_categories', $data['id'] ) );
		...
		if ( ( $data['parentid'] != $data['parentdb'] )
			...
			//LOCK TABLE #__jseblod_cck_searchs_categories WRITE; //todo//lock//
			...
			if ( $key == 0 ) {
				$query	= ' SELECT lft FROM #__jseblod_cck_searchs_categories'
						. ' WHERE name = "'.$parent.'"'
						;
			} else {
				$bigbrother	=	$brothers[$key - 1];
				$query	= ' SELECT rgt FROM #__jseblod_cck_searchs_categories'
			...
			$query	= 'UPDATE #__jseblod_cck_searchs_categories'
	...
	function delete( $deleteMode )
	{
		...
		if ( $n = count( $cids ) )
		{
			foreach( $cids as $cid ) {
				...
				$query	= 'SELECT lft, rgt, ( rgt - lft + 1 ) AS width FROM #__jseblod_cck_searchs_categories'
						. ' WHERE id = '.$cid
						;   
				$this->_db->setQuery( $query );
				...
				if ( $deleteMode == -1 ) {
					if ( ! ( $route[1] - $route[0] == 1 ) ) {				  
						$query	= 'DELETE FROM #__jseblod_cck_searchs_categories'
