...
/**
 * Types_Categories		Model Class
 **/
class CCKjSeblodModelTypes_Categories extends JModel
{
	/**
	 * Vars
	 **/
	...
	function getData()
	{
		if ( empty( $this->_data ) )
		{
			...
			$contentTypes	=	$this->_db->loadObjectList( 'category' );
			if ( sizeof( $this->_data ) ) {
				foreach ( $this->_data as $item ) {
					$item->displayTypes	=	@$contentTypes[$item->id]->num ? $contentTypes[$item->id]->num : 0;
				}	
			}
		}
