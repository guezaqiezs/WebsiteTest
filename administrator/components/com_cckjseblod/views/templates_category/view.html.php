...
/**
 * Templates_Category			View Class
 **/
class CCKjSeblodViewTemplates_Category extends JView
{
	/**
	...
	{
		JToolBarHelper::title(   JText::_( 'TEMPLATE CATEGORY' ).': <small><small>[ '.JText::_( 'Delete' ).' ]</small></small>', 'category.png' );
		JToolBarHelper::custom( 'cancel', 'cancel_jseblod', 'cancel_jseblod', JText::_( 'Close' ), false ); //JToolBarHelper::cancel( 'cancel', 'Close' );
		JToolBarHelper::divider();
		HelperjSeblod_Display::help( 'templates_categories' );
	}
	...
	function _displayToolbar( $isNew, $doCopy, $isAuth )
	{
		...
		if ( $isNew || $doCopy )  {
			...
			JToolBarHelper::title(   JText::_( 'TEMPLATE CATEGORY' ).': <small><small>[ '.$text.' ]</small></small>', 'category.png' );
		...
		JToolBarHelper::divider();
		HelperjSeblod_Display::help( 'templates_category' );
	}
	...
	function display( $tpl = null )
	{
		...
		if ( $doDelete ) {
			...
			if ( JTable::isCheckedOut( $user->get( 'id' ), $category->checked_out ) ) {
				...
				$mainframe->redirect( _LINK_CCKJSEBLOD_TEMPLATES_CATEGORIES, $msg, 'notice' );
			}
			...
			$optParents		=	array_merge( $optParents, HelperjSeblod_Helper::getTemplateCategories( false, false ) );
