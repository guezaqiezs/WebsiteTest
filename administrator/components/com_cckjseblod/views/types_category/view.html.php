...
/**
 * Types_Category			View Class
 **/
class CCKjSeblodViewTypes_Category extends JView
{
	/**
	...
	{
		JToolBarHelper::title(   JText::_( 'TYPE CATEGORY' ).': <small><small>[ '.JText::_( 'Delete' ).' ]</small></small>', 'category.png' );
	...
	function _displayToolbar( $isNew, $doCopy, $isAuth )
	{
		...
		if ( $isNew || $doCopy )  {
			...
			JToolBarHelper::title(   JText::_( 'TYPE CATEGORY' ).': <small><small>[ '.$text.' ]</small></small>', 'category.png' );
		...
		JToolBarHelper::divider();
		HelperjSeblod_Display::help( 'types_category' );
	}
	...
	function display( $tpl = null )
	{
		...
		if ( $doDelete ) {
			...
			if ( JTable::isCheckedOut( $user->get( 'id' ), $category->checked_out ) ) {
				...
				$mainframe->redirect( _LINK_CCKJSEBLOD_TYPES_CATEGORIES, $msg, 'notice' );
			}
			...
			$optParents		=	array_merge( $optParents, HelperjSeblod_Helper::getTypeCategories( false, false ) );
