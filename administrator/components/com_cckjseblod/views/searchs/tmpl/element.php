...
$empty	=	JText::_( 'SELECT A SEARCH BY' );
$buttons = array('Empty'	=> array( 'Empty', 'trash', "javascript: window.parent.jEmptySearch('$empty', '$this->object' );", 'onclick' ),
...
<div id="modal-top">
	<fieldset class="adminform modal-bg-toolbar">
		<div class="header icon-48-searchs" style="float: left">
			<?php echo JText::_( 'SEARCH TYPES' ) . ': <small><small>[ '.JText::_( 'SELECT' ).' ]</small></small>'; ?>
		</div>
	...
	<fieldset class="adminform">
		...
		<div id="editcell">
			...
			<?php
			$k = 0;
			for ( $i = 0, $n = count( $this->searchs_items ); $i < $n; $i++ )
			{
				...
				<tr class="<?php echo "row$k"; ?>">
					...
					<td align="left">
						...
						<div class="title-left">
							<span class="editlinktip hasTip" title="<?php echo JText::_( 'SELECT SEARCH' ); ?>::<?php echo $row->title; ?>">	
								<?php echo _NBSP; ?><a href="#" onclick="window.parent.jSelectSearch('<?php echo $row->id; ?>', '<?php echo addslashes($row->title); ?>', '<?php echo $this->object; ?>');"><?php echo $row->title; ?></a>
							</span>
