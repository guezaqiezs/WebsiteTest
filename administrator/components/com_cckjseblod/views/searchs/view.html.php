...
/**
 * Content Search	View Class
 **/
class CCKjSeblodViewSearchs extends JView
{
	/**
	 * Display Toolbar
	 **/
	...
	{
		JToolBarHelper::title(   JText::_( 'SEARCH TYPE MANAGER' ), 'searchs.png' );
		if ( $categoryFilter ) {
	...
	function display( $tpl = null )
	{
		...
		$searchs_items		=	$model->getData( $layout, $action );
    
		// Set Flags
		$isAuth = ( $user->get( 'gid' ) < _EDIT_ACCESS ) ? 0 : 1;
		
		// Get User State
		$filter_restricted	= $mainframe->getUserStateFromRequest( $option.'.'.$controller.'.'.$task.'.filter_restricted', 'filter_restricted', 1,		'int' );
		...
		// Set Search Filter
		$options_search[] 		= JHTML::_( 'select.option', '<OPTGROUP>', JText::_( 'SEARCH TYPES' ) );
		...
		$options_search[] 		= JHTML::_( 'select.option', '<OPTGROUP>', JText::_( 'SEARCH TYPE CATEGORIES' ) );
		...
		$optionCategories	= array_merge( $optionCategories, HelperjSeblod_Helper::getSearchCategories( false, true ) );
		...
		$optionCategories[]	= JHTML::_( 'select.option',  '1', JText::_( 'QUICK CATEGORY' ), 'value', 'text' );
		$optionCategories[] = JHTML::_( 'select.option', '<OPTGROUP>', JText::_( 'Categories' ) );
		$optionCategories	= array_merge( $optionCategories, HelperjSeblod_Helper::getSearchCategories( true, true ) );
		$optionCategories[]	= JHTML::_( 'select.option', '</OPTGROUP>', '' );
		...
		$this->assignRef( 'searchs_items', $searchs_items );
