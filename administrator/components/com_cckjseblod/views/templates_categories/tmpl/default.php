...
<fieldset>
<legend class="legend-border"><?php echo JText::_( 'TEMPLATE CATEGORIES' ) .' :: '. $stats; ?></legend>
<table>
	<tr>
...
<div id="editcell">
	...
	for ( $i = 0; $i < $n; $i++ )
	{
		...
		$mychecked		=	( $row->id == 1 || $row->id == 2 || $row->id == 3 || $row->id == 4 || $row->id == 5 || $row->id == 6 || $row->id == 7 || $row->id == 8 || $row->name == 'list_autolist' ) ? '<img src="images/checked_out.png" alt="X" />' : JHTML::_( 'grid.checkedout', $row, $i );		
		...
		$linkTemplates		=	JRoute::_( 'index.php?option='.$this->option.'&controller=templates&categoryfilter='.$row->id );
		$displayTemplates	=	( $row->displayTemplates ) ? '<span class="editlinktip hasTip" title="'.JText::_( 'EDIT TEMPLATES' ).'::'.$row->title.'"><a href="'.$linkTemplates.'">'._IMG_TEMPLATES_24.'</a></span>' : '-';
		...
		<tr class="<?php echo "row$k"; ?>">
			...
			<td align="center" width="4%" <?php echo $borderNoname; ?>>
				<?php echo $displayTemplates; ?>
			</td>
			<td align="center" width="16%" <?php echo $borderNoname; ?>>
				<?php echo ( $row->displayTemplates ) ? $row->displayTemplates . '&nbsp;' . ( ( $row->displayTemplates == 1 ) ? JText::_( 'TEMPLATE' ) : JText::_( 'TEMPLATES' ) ) : '-'; ?>
			</td>
			<td align="center" <?php echo $borderNoname; ?>>
				<?php echo $published; ?>
			</td>
