...
<fieldset>
<legend class="legend-border"><?php echo JText::_( 'ITEM CATEGORIES' ) .' :: '. $stats; ?></legend>
<table>
	<tr>
...
<div id="editcell">
	...
	<thead>
		<tr>
			...
			<th class="title" width="20%" colspan="2">
				<?php echo '#&nbsp;' . JText::_( 'CONTENT ITEMS' ); ?>
			</th>
			<th class="title" width="10%">
	...
	for ( $i = 0; $i < $n; $i++ )
	{
		...
		$mychecked		=	( $row->id == 1 || $row->id == 2 || $row->id == 3 || $row->id == 4 || $row->id == 5 || $row->id == 6 || $row->id == 7 || $row->id == 8 || $row->id == 9 || $row->id == 10 ) ? '<img src="images/checked_out.png" alt="X" />' : JHTML::_( 'grid.checkedout', $row, $i );		
		...
		$linkItems			=	JRoute::_( 'index.php?option='.$this->option.'&controller=items&categoryfilter='.$row->id );
		$displayItems		=	( $row->displayItems ) ? '<span class="editlinktip hasTip" title="'.JText::_( 'EDIT CONTENT ITEMS' ).'::'.$row->title.'"><a href="'.$linkItems.'">'._IMG_ITEMS_24.'</a></span>' : '-';
		$displayMode		=	( $row->display % 2 == 0 ) ? ( ( $row->display == 0 ) ? JText::_( 'Hide' ) : JText::_( 'ON CONTENT ITEM LIST VIEW' ) ) : ( ( $row->display == 3 ) ? JText::_( 'ALWAYS' ) : JText::_( 'ON CONTENT TYPE EDIT VIEW' ) );
		...
		<tr class="<?php echo "row$k"; ?>">
			...
			<td align="center" <?php echo $borderNoname; ?>>
				<span class="editlinktip hasTip" title="<?php echo JText::_( 'FILTER BY CATEGORY' ); ?>::<?php echo $row->title; ?>">
					<a href="<?php echo $linkFilterCategory; ?>" border="0" style="text-decoration: none;">
			...
			<td align="center" width="16%" <?php echo $borderNoname; ?>>
				<?php echo ( $row->displayItems ) ? $row->displayItems . '&nbsp;' . ( ( $row->displayItems == 1 ) ? JText::_( 'ITEM' ) : JText::_( 'ITEMS' ) ) . '<br />' . $displayMode : '-'; ?>
			</td>
			<td align="center" <?php echo $borderNoname; ?>>
				<?php echo $published; ?>
			</td>
