...
<fieldset>
<legend class="legend-border"><?php echo JText::_( 'TYPE CATEGORIES' ) .' :: '. $stats; ?></legend>
<table>
	<tr>
...
<div id="editcell">
	...
	for ( $i = 0; $i < $n; $i++ )
	{
		...
		$mychecked		=	( $row->id == 1 || $row->id == 2 || $row->id == 3 || $row->id == 4 || $row->id == 5 || $row->id == 6 ) ? '<img src="images/checked_out.png" alt="X" />' : JHTML::_( 'grid.checkedout', $row, $i );		
		...
		$linkTypes			=	JRoute::_( 'index.php?option='.$this->option.'&controller=types&categoryfilter='.$row->id );
		$displayTypes		=	( $row->displayTypes ) ? '<span class="editlinktip hasTip" title="'.JText::_( 'EDIT CONTENT TYPES' ).'::'.$row->title.'"><a href="'.$linkTypes.'">'._IMG_TYPES_24.'</a></span>' : '-';
		$displayMode		=	( $row->display % 2 == 0 ) ? ( ( $row->display == 0 ) ? JText::_( 'Hide' ) : JText::_( 'ON CONTENT TYPE LIST VIEW' ) ) : ( ( $row->display == 3 ) ? JText::_( 'ALWAYS' ) : JText::_( 'ON CONTENT ITEM LIST VIEW' ) );
		...
		<tr class="<?php echo "row$k"; ?>">
			...
			<td align="center" width="16%" <?php echo $borderNoname; ?>>
				<?php echo ( $row->displayTypes ) ? $row->displayTypes . '&nbsp;' . ( ( $row->displayTypes == 1 ) ? JText::_( 'TYPE' ) : JText::_( 'TYPES' ) ) . '<br />' . $displayMode : '-'; ?>
			</td>
			<td align="center" <?php echo $borderNoname; ?>>
				<?php echo $published; ?>
			</td>
