...
$empty	=	JText::_( 'SELECT A TEMPLATE BY' );
$buttons = array('Empty'	=> array( 'Empty', 'trash', "javascript: window.parent.jEmptyTemplate('$empty', 'templateid' );", 'onclick' ),
...
<div id="modal-top">
	<fieldset class="adminform modal-bg-toolbar">
		<div class="header icon-48-templates" style="float: left">
			<?php echo JText::_( 'TEMPLATES' ) . ': <small><small>[ '.JText::_( 'SELECT' ).' ]</small></small>'; ?>
		</div>
	...
	<fieldset class="adminform">
		<table>
			<tr>
				<td align="left" width="100%">
					...
					<button onclick="this.form.getElementById('filter_mode').value='';this.form.getElementById('filter_category').value='0';this.form.submit();"><?php echo JText::_( 'Search' ); ?></button>
					<button onclick="document.getElementById('search').value='';this.form.getElementById('filter_search').value='0';this.form.getElementById('filter_mode').value='';this.form.getElementById('filter_category').value='0';this.form.submit();"><?php echo JText::_( 'Reset' ); ?></button>
				</td>
				<td nowrap="nowrap">
		...
		<div id="editcell">
			...
			<?php
			$k = 0;
			for ( $i = 0, $n = count( $this->templates_items ); $i < $n; $i++ )
			{
				$row 		=&	$this->templates_items[$i];
				$published 	= ( $row->published ) ? '<img src="images/tick.png" />' : '<img src="images/publish_x.png" />';
				...
				<tr class="<?php echo "row$k"; ?>">
					...
					<td align="left">
						...
						<span class="editlinktip hasTip" title="<?php echo JText::_( 'SELECT TEMPLATE' ); ?>::<?php echo $row->title; ?>">	
							<?php echo _NBSP; ?><a href="#" onclick="window.parent.jSelectTemplate('<?php echo $row->id; ?>', '<?php echo addslashes($row->title); ?>', 'templateid');"><?php echo $row->title; ?></a></span>
			...
			<tfoot>
				<tr>
					...
					<td colspan="4" id="pagination-bottom"><?php echo $this->pagination->getListFooter(); ?></td>
					<td><a href="#modal-top" style="color: gray; text-decoration: none;">&nbsp;<img src="images/sort_asc.png" />&nbsp;</a></td>
				</tr>
			</tfoot>
			</table>
