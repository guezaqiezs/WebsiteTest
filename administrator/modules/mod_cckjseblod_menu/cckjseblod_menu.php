...
class JAdminCSSCCKMenu extends JTree
{
	...
	function renderMenu($id = 'cckjseblod_menu', $class = 'cckjseblod_menu')
	{
		global $mainframe;

		if (!$class) $class='cckjseblod_menu';
	...
	function renderLevel($depth)
	{
		...
		if ($this->_current->link != null) {
			if ( strpos( $this->_current->link, 'http' ) !== false ) {
				echo "<a class=\"".$this->getIconClass($this->_current->class)."\" target=\"_blank\" href=\"".$this->_current->link."\">".$this->_current->title."</a>";
			} else {
		...
		while ($this->_current->hasChildren())
		{
			if ($this->_current->class) {
				echo '<ul id="cckjseblod_menu-'.strtolower($this->_current->id).'"'.
					' class="cckjseblod_menu-component">'."\n";
			} else {
				echo '<ul>'."\n";
			}
			foreach ($this->_current->getChildren() as $child)
