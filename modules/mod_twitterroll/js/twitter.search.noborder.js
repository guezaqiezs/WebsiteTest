...
;(function($) {
	$.fn.twitterSearchesN = function(options) {
		...
		return this.each(function() {
			...
			$title = $('<div class="twitterSearchesNTitle"></div>').append($text).appendTo($frame).css(opts.css['title']);
			if (opts.bird) {
	...
	$.fn.twitterSearchesN.formatter = function(json, opts) {
		...
		if (opts.avatar)
			str += '<img class="twitterSearchesNProfileImg" src="' + json.profile_image_url + '" />';
		str += '<div><span class="twitterSearchesNUser"><a href="http://www.twitter.com/'+ json.from_user+'"/ TARGET="_blank">' 
	...
	$.fn.twitterSearchesN.defaults = {
		...
		css: {
			...
			title: { margin: 0, padding: '0 0 5px 0', textAlign: 'center', fontWeight: 'bold', fontSize: 'large', position: 'relative' },
