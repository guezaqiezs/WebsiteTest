 ...
 * Modified for Joomla by: GraphicAholic.com
 */
 
;(function($) {
	$.fn.twitterSearchesB = function(options) {
		...
		return this.each(function() {
			var grabFlag = false,
				...
				opts = $.extend(true, {}, $.fn.twitterSearchesB.defaults, options || {}, $.metadata ? $frame.metadata() : {});
				
			opts.formatter = opts.formatter || $.fn.twitterSearchesB.formatter; 
			opts.filter = opts.filter || $.fn.twitterSearchesB.filter;
			...
			text = opts.titleLink ? ('<a href="'+ opts.titleLink +'"/ TARGET="_blank">'+ opts.title + '</a>') : ('<span>' + opts.title + '</span>');
			$text = $(text);
			if (opts.titleLink)
				$text.css(opts.css['titleLink']);
			$title = $('<div class="twitterSearchesBTitle"></div>').append($text).appendTo($frame).css(opts.css['title']);
			if (opts.bird) {
				$bird = $('<img class="twitterSearchesBBird" src="'+opts.birdSrc+'" />').appendTo($title).css(opts.css['bird']);
				if (opts.birdLink)
					$bird.wrap('<a href="'+ opts.birdLink +'"></a>');
			}
			$cont = $('<div class="twitterSearchesBContainter"></div>').appendTo($frame).css(opts.css['container']);
			cont = $cont[0];
			...
			$('<div class="twitterSearchesBLoading">Loading tweets..</div>').css(opts.css['loading']).appendTo($cont);
					...
					success: function(json) {
						...
						$cont.fadeOut('fast',function() {
							...
							$.each(json.results, function(i) {
								...
								$img = $tweet.find('.twitterSearchesBProfileImg').css(opts.css['img']);
								$tweet.find('.twitterSearchesBUser').css(opts.css['user']);
								$tweet.find('.twitterSearchesBTime').css(opts.css['time']);
								$tweet.find('a').css(opts.css['a']);
								$tweet.appendTo($cont);
								$text = $tweet.find('.twitterSearchesBText').css(opts.css['text']);
								if (opts.avatar) {
			...
			function failWhale(msg) {
				var $fail = $('<div class="twitterSearchesBFail">' + msg + '</div>').css(opts.css['fail']);
	...
	$.fn.twitterSearchesB.filter = function(tweet) {
		return true;
	};

	$.fn.twitterSearchesB.formatter = function(json, opts) {
		var str, pretty,
			text = json.text;
		if (opts.anchors) {
		...
		str = '<div class="twitterSearchesBTweet">';
		if (opts.avatar)
			str += '<img class="twitterSearchesBProfileImg" src="' + json.profile_image_url + '" />';
		str += '<div><span class="twitterSearchesBUser"><a href="http://www.twitter.com/'+ json.from_user+'"/ TARGET="_blank">' 
		  + json.from_user + '</a></span>';
		pretty = prettyDate(json.created_at);
		if (opts.time && pretty)
			str += ' <span class="twitterSearchesBTime">('+ pretty +')</span>'
		 str += '<div class="twitterSearchesBText">' + text + '</div></div></div>';
		 return str;
	};
	
	$.fn.twitterSearchesB.defaults = {
